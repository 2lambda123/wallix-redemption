...
struct TestWidgetLabelCtx
{
    struct Colors
    {
        BGRColor fg = NamedBGRColor::RED;
        BGRColor bg = NamedBGRColor::YELLOW;
    };
    ...
    TestWidgetLabelCtx(
        chars_view text, Colors colors = Colors{NamedBGRColor::RED, NamedBGRColor::YELLOW}, int xtext = 0, int ytext = 0)
    : wlabel(
        drawable, text,
...
RED_AUTO_TEST_CASE(TraceWidgetLabelMax)
{
    ...
    RED_CHECK(ctx.wlabel.get_text() == text.subarray(0, text.size()-2));
